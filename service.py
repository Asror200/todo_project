...
@is_authenticated
def get_all_todos():
    todos = [] # databazadan kelgan todolarni saqlab update_todo funksiyasiga yuborish uchun ishlatildi
...
def todo_update(name: str, todo_id: int, description: Optional[str] = None):
    todos = get_all_todos()
    for todo in todos:
        if todo_id in str(todo): # inputdan harf yoki bo'sh malumot kiritilganda xatolik bermasligi uchun str qilib olindi
